...
Page({
   ...
   gotoBuy () {
      if (app.globalData.userInfo === null) {
         wx.navigateTo({
            url: `/pages/result/result?page=login&id=${this.data.detailData.info.id}&from=ticket`
         })
      } else {
   ...
   getRecListData () {
      ...
      wx.showNavigationBarLoading()
      wx.request({
         url: app.globalData.ajaxSrc + '/recommend_list',
         data: {
            keyword: self.data.keywords,
            page: self.data.page
         },
   ...
   bannerGoOut () {
      ...
      let type = this.data.bannerType
      let src = '/pages/drink/index'
      if (type == 1) {
