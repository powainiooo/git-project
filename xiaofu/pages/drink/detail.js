...
Page({
   ...
   doPay () {
      if (app.globalData.userInfo === null) {
         wx.navigateTo({
            url: `/pages/result/result?page=login&id=${this.data.orderID}&from=drink`
         })
      }
      let self = this
   ...
   */
   ...
   drawPoster () {
      ...
      Promise.all([
         ...
         ctx.setFontSize(26)
         let len = gblen(data.drink_name)
         let row = Math.ceil(len / 14)
         for (let i = 0; i < row; i++) {
      ...
      wx.downloadFile({
         ...
         success: function (res) {
            if (res.statusCode === 200) {
               ...
               ctx.setFillStyle('#000')
               ctx.setFontSize(26)
               let len = data.drink_name.length
               let row = Math.ceil(len / 9)
               for (let i = 0; i < row; i++) {
