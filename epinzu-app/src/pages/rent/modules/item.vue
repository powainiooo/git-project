...
<view class="Rent-item section">
  ...
  <view class="item borderB" v-for="item in record.goods" :key="item.id">
    ...
    <view class="end btns mt16">
      ...
      <button class="c-btn c-btn-border c-btn-24 ml4" v-if="item.buttons.account_confirm === 1" @tap.stop="confirm">确认账单</button>
      <button class="c-btn c-btn-border c-btn-24 ml4" v-if="item.buttons.account_view === 1" @tap.stop="toDetail">账单详情</button>
    </view>
  </view>
</view>
</template>
